...
export async function query<T>(
  ...
  try {
    ...
    try {
      ...
      if (duration > 100) {
        logger.warn(`🐘 PostgreSQL SLOW_QUERY (${duration}ms): [Tenant: ${tenantId}] ${text.substring(0, 100)}...`);
      }
    ...
    logger.error(`🐘 PostgreSQL CONNECTION_FAILURE (${err.code || err.severity}): ${err.message} [Tenant: ${tenantId}]`);
    throw err;
  }
}
...
export async function transaction<T>(
  ...
  try {
    ...
    try {
      ...
      const duration = Date.now() - start;
      if (duration > 500) {
        logger.warn(`🐘 PostgreSQL SLOW_TRANSACTION (${duration}ms): [Tenant: ${tenantId}]`);
      }
      return result;
    } catch (e) {
