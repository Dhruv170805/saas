...
import { fmtPrice as formatPrice } from '@/core/format'
import { useSettings, useCustomers, useTenant, usePOSMutations } from '@/hooks/useData'
import { OrderDetailSkeleton } from '@/components/ui/Skeletons'
import { useQuery } from '@tanstack/react-query'
import { orderService } from '@/services/api'

import type { AppSettings, Order } from '@/core/db'
import type { DbOrderItem } from '@/core/db/schema'
import { getOrderSharingLinks } from '@/services/social'
...
export default function OrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: order, isLoading: orderLoading } = useQuery({
    ...
    queryFn: async () => {
      const res = await orderService.getOrder(Number(resolvedParams.id))
  ...
  const { updateOrderStatus } = usePOSMutations()
  ...
  const handleStatusUpdate = async (newStatus: string, paymentMethod?: string) => {
    try {
      ...
      await updateOrderStatus.mutateAsync({ 
        ...
        customerName: paymentMethod === 'UNPAID' ? customerName : undefined,
        customerPhone: paymentMethod === 'UNPAID' ? customerPhone : undefined
      })
  ...
  const shareLinks = getOrderSharingLinks(order, { name: tenant?.name, config: settings })
    ...
    <div className="max-width-lg mx-auto p-4">
      ...
      <div className="receipt-card print-area">
        ...
        <div style={{ textAlign: 'center', paddingBottom: '0.5rem', borderBottom: '2px solid #000', marginBottom: '0.75rem' }}>
          ...
          {settings?.address && (
            <div style={{ fontSize: '0.72rem', marginTop: '0.1rem' }}>{settings.address}</div>
          )}
          {settings?.phone && (
            <div style={{ fontSize: '0.72rem' }}>Ph: {settings.phone}</div>
          )}
        </div>
        ...
        <div style={{ textAlign: 'center', marginTop: '0.75rem', paddingTop: '0.5rem', borderTop: '1px dashed #000', fontSize: '0.8rem' }}>
          <p style={{ fontWeight: 700 }}>{settings?.tagline || 'Thanks!'}</p>
        </div>
