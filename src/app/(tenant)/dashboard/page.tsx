...
import { useDashboard, useSettings, useTenant } from '@/hooks/useData'
import type { Order } from '@/core/db'
import {
  AreaChart,
  Area,
  XAxis,
...
function buildHourlyData(raw?: Record<number, number>) {
                        ...
                        {order.customerPhone && (
                          <a
                            href={`https://wa.me/${order.customerPhone.replace(/\D/g, '')}?text=${encodeURIComponent(
                              `Hello ${order.customerName || 'there'},\n\nReminder from ${tenant?.name || 'us'}: your bill of ${fmtPrice(total)} for Order #${order.id} is unpaid. Please settle at your earliest convenience.\n\nView Bill: ${window.location.origin}/bill/${order.id}\n\nThank you!`
                            )}`}
                            target="_blank"
