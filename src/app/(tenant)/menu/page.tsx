  ...
  const handleSubmitItem = async () => {
    ...
    try {
      ...
      if (formCategoryId === 'NEW') {
        const catRes = await menuService.createCategory({ name: newCategoryName.trim() })

        if (catRes.error) {
          toast.error(catRes.error || 'Failed to create new category')
          setSubmitting(false)
          return
        }
        ...
        finalCategoryId = formCategoryId as number
      }
