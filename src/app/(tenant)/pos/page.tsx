...
import { calculateTaxWithSettings } from '@/core/pricing'
import type { MenuItem } from '@/core/db'
...
function POSContent() {
  ...
  const checkoutMutation = useMutation({
    mutationFn: async () => {
      ...
      if (orderId) {
        return orderService.addOrderItems(orderId, items)
      } else {
  ...
  const handleCheckout = () => {
    if (cart.length === 0 || checkoutMutation.isPending || !tableNumber) return
    ...
    <div className="flex gap-6 pos-layout" style={{ height: 'calc(100vh - 120px)' }}>
      ...
      <div className="card flex flex-col pos-cart" style={{ width: '380px', height: '100%' }}>
        ...
        <div style={{ borderTop: '1px solid var(--glass-border)', paddingTop: '1rem', marginTop: '1rem' }}>
          ...
          <button className="checkout-btn" onClick={handleCheckout} disabled={cart.length === 0 || checkoutMutation.isPending}>
            {checkoutMutation.isPending ? 'Processing...' : orderId ? `Add to Order #${orderId}` : `Place Order — Table ${tableNumber}`}
          </button>
        </div>
      </div>
    </div>
