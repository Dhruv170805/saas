import { getTables } from '@/core/db/tables'
import { getOrder } from '@/core/db/orders'
import { getTenantBySlug } from '@/core/db/tenants'
import { headers } from 'next/headers'
import { fmtPrice } from '@/core/format'
...
export default async function CustomerTableOrderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const order = await getOrder(table.order.id, tenant.id)
  if (!order) {
    return (
      <div className="max-width-lg mx-auto p-4 text-center">
        <PageHeader title={`Table ${tableNumber}`} />
